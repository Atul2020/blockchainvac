...
class Blockchain:
    ...
    def add_transaction(self, Donor, Receiver, Name, time):
        self.transactions.append({'Donor': Donor,
                                  'Receiver': Receiver,
                                  'Name': Name,
...
def mine_block(request):
    if request.method == 'GET':
        ...
        blockchain.add_transaction(Donor = root_node, Receiver = node_address, Name= 'genesys', time=str(datetime.datetime.now()))
...
def add_transaction(request):
    if request.method == 'POST':
        ...
        transaction_keys = ['Donor', 'Receiver', 'Name','time']
        if not all(key in received_json for key in transaction_keys):
            return 'Some elements of the transaction are missing', HttpResponse(status=400)
        index = blockchain.add_transaction(received_json['Donor'], received_json['Receiver'], received_json['Name'],received_json['time'])
